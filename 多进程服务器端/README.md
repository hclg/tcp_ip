...
多进程服务端的缺点，对每个客户端都要创建进程，占用大量的运算和内存，而且通信还需要IPC（管道）相对复杂的通信方法。
...
*  对于为什么要多次销毁套接字文件描述符。
* **调用fork()函数后子进程复制了父进程的所有资源，而导致父进程和子进程的两个文件描述符指向同个套接字。
* 而只有套接字上所有文件描述符都销毁才能销毁套接字
* 所以我们这里选择父进程先删除对客户端的套接字，子进程删除对服务端的套接字文件描述符，这样就保证一一对应了，无关的文件描述符都关了。

#### 5. 分割TCP的I/O程序

​	为了实现在发送给服务端数据后，不必等待服务端的回复就可以发送下一个数据，而把输入输出分开为两个进程让程序在实现上更为简单。
...
因为是回声程序，简单输入看不出区别，可以依照上面的样例输入输出。